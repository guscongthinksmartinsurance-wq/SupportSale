...
def load_data(worksheet):
    try:
        ...
        # Tự động tạo khung cột đúng chuẩn Baseline của anh
        if worksheet == "leads":
...
# --- 5. RENDER DANH SÁCH (FULL BASELINE) ---
for idx, row in df_leads.iterrows():
    ...
    with st.container(border=True):
        ...
        with c_note:
            ...
            # Nhập Note Ăn Ngay
            new_txt = st.text_input("Ghi chú & Enter", key=f"n_{idx}", placeholder="Note nhanh...", label_visibility="collapsed")
            if new_txt:
                now = datetime.now()
        ...
        with c_edit:
            with st.popover("⋮"):
                ...
                if st.button("Xóa khách", key=f"del_kh_{idx}", type="primary", use_container_width=True):
                    full_df = load_data("leads")
                    save_and_refresh(full_df.drop(idx).reset_index(drop=True), "leads")
        st.divider()
